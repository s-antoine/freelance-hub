  ...
  <div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-8">
          ...
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="text-right px-4 py-3 text-xs text-gray-500 font-medium w-24">Qté</th>
                <th class="text-right px-4 py-3 text-xs text-gray-500 font-medium w-32">Prix unit.</th>
                <th class="text-right px-4 py-3 text-xs text-gray-500 font-medium w-32">Total</th>
                <th class="w-10"></th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(item, i) in items" :key="i">
                <td class="px-4 py-2">
                  <input v-model="item.description" type="text" placeholder="Description" class="w-full text-sm border-0 focus:outline-none focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1" />
                </td>
                <td class="px-4 py-2">
                  <input v-model.number="item.quantity" type="number" min="0" step="0.5" class="w-full text-sm text-right border-0 focus:outline-none focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1" />
                </td>
                <td class="px-4 py-2">
                  <input v-model.number="item.unit_price" type="number" min="0" step="0.01" class="w-full text-sm text-right border-0 focus:outline-none focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1" />
                </td>
      ...
      <div class="flex justify-end mb-8">
        <div class="w-64 space-y-2">
          ...
          <div class="flex items-center justify-between text-sm">
            ...
            <input v-model.number="form.tax_rate" type="number" min="0" max="100" step="0.5" class="w-20 text-right border border-gray-200 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">TVA</span>
      ...
      <div class="flex gap-3">
        ...
        <button @click="save('sent')" :disabled="saving" class="btn-primary ml-auto">
